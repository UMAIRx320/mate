...
for x in range(9999):
	...
	h1 = f"Mozilla/5.0 (Linux; Android {str(random.randint(2,8))}.{str(random.randint(1,9))}.{str(random.randint(1,9))}; LG-K520 Build/{builx}; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/95.0.{chrome4}.{chrome3} Mobile Safari/537.36[FBAN/EMA;FBLC/en_US;FBAV/309.0.0.16.{chrome3};]"
	entot.append(h1)
	
	rr = random.randint
	andro = f"{(rr(4,13))}"
	memek = f"Dalvik/2.1.0 (Linux; U; Android {andro}; TRT-AL00 Build/HUAWEITRT-AL00) [FBAN/Orca-Android;FBAV/{(rr(100,467))}.0.0.16.158;FBPN/com.facebook.orca;FBLC/en_US;FBBV/{(rr(100000000,9000000000))};FBCR/null;FBMF/huawei;FBBD/huawei;FBDV/TRT-AL00;FBSV/{andro};FBCA/armeabi-v7a:armeabi;FBDM/"+"{density=3.0,width=1080,height=1920"
	...
	h2 = f"Mozilla/5.0 (Linux; U; Android {android}; Redmi 5A Build/{build}.H1; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/{chrome} Mobile Safari/537.36 OPR/{browser}"
	entot.append(h2)
...
def crack(idf,pwv):
	...
	fff = '%'
	prog.update(des,description=f'\r{A}[{M}LOVE{A}] {P}[deep_white]{(loop)}/{len(id)}[/] {x}[green]OK[/]:[green]{(ok)} [/]=[yellow] CP[/]:[yellow]{(cp)}')
	prog.advance(des)
